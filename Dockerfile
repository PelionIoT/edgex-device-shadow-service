...
EXPOSE 17362/tcp
RUN apt-get update
RUN apt-get -y install openjdk-8-jre vim sudo locales openssh-server supervisor dnsutils unzip zip mosquitto git libmosquitto-dev mosquitto-clients libc6-dev build-essential cmake git doxygen graphviz jq curl socat
